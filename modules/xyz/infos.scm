...
(define (infos->region infos)
  "- Scheme Procedure: infos->region infos
...
(define (infos->list infos)
  "- Scheme Procedure: infos->list infos
    Convert an infos-bog to a list"
  (if (not (null? infos))
      (list
       (assq-ref infos 'name)
       (list
...
(define (xyz-set-infos! infos x1 y1 z1)
  (let ((xmin (assq-ref infos 'xmin)) (xmax (assq-ref infos 'xmax))
    ...
    (if (< x1 xmin) 
	(assq-set! infos 'xmin x1)
	(if (> x1 xmax) 
	    (assq-set! infos 'xmax x1)))
    (if (< y1 ymin) 
	(assq-set! infos 'ymin y1)
	(if (> y1 ymax) 
	    (assq-set! infos 'ymax y1)))
    (if (< z1 zmin) 
	(assq-set! infos 'zmin z1)
	(if (> z1 zmax) 
	    (assq-set! infos 'zmax z1))))
  (assq-set! infos 'count (1+ (assq-ref infos 'count))))

(define* (xyz->infos xyz-list #:optional (infos #f))
  "- Scheme Procedure: xyz->infos xyz-list #:infos
    Snarf an infos blob from an xyz-list."
  (cond
   ((null? xyz-list) infos)
   ((not infos)
...
(define* (xyz-port->infos #:optional 
			  ...
			  (infos #f) 
			  #:key (test #f))
  "- Scheme Procedure: xyz-port->infos #:port #:infos #:test
    Snarf an infos blob from an xyz-port."
  (define xyz-infos
    (lambda (xyz)
      (if (or (not test) 
	      (test xyz))
...
(define* (inf->infos info-port #:optional (close? #f) (infos '()))
  "- Scheme Procedure: inf->infos info-port #:close? #:info
    Convert an MBSystem .inf file to an infos blob"
