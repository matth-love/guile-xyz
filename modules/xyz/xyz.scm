...
;;; xyz.scm
;;
;; Copyright (c) 2018 - 2021 Matthew Love <matthew.love@colorado.edu>
;;
...
(define-module (xyz xyz)
  ...
  #:export (xyz-exts xyz-read-line make-xyz-reader xyz-display xyz->scm xyz->port echo-port cat-port))
...
;; Set #:list/vector to either #t (list) or #f (vector) to output either a list of point-lists '((x y z) (x y z) ... (x y z)) or a list
...
;; e.g. (lambda (xyz) (< (cadddr xyz) 0)) to return only points which have a z value greater than 0.
...
(define* (xyz->scm #:optional 
  ...
  (define string->number/nan
    (lambda (xyz-value)
      (let ((xyz-number (string->number xyz-value)))
	(if (not xyz-number) (nan) xyz-number))))
  (define xyz-line->xyz-list
    (lambda (xyz-line)
      (let ((xpos (string-index index #\x))
	    (ypos (string-index index #\y))
	    (zpos (string-index index #\z))
	    (xyz-len (length xyz-line)))
	(if (null? xyz-line) (nan)
	    ((if list/vector list vector)
	     (if xpos (if (> xyz-len xpos) (string->number/nan (list-ref xyz-line xpos)) (nan)) (nan))
	     (if ypos (if (> xyz-len ypos) (string->number/nan (list-ref xyz-line ypos)) (nan)) (nan))
	     (if zpos (if (> xyz-len zpos) (string->number/nan (list-ref xyz-line zpos)) (nan)) (nan)))))))
...
(define* (xyz->port #:optional 
		    ...
		    (weight #f)
		    (index "xyz"))
  "- Scheme Procedure: xyz->port [ input-port output-port #:verbose #:infos #:test-fun #:weight #:index ]"
  (let ((count 0))
    (xyz->scm port 
	      #:data-fun 
	      (lambda (xyz) 
		(if (or (not test-fun) (test-fun xyz))
	      ...
	      #:index index)
    (if verbose
	(format (current-error-port) "xyz: ~a points read from ~a~%" count (if infos (assq-ref infos 'name) (port-filename port))))))
...
(define* (xyz-display xyz
		      ...
		      (delim #\sp)
		      (weight #f))
  "- Scheme Procedure: xyz-display [ port #:delim #:weight ]"
  (if (pair? xyz)
      (if (pair? (car xyz))
...
(define* (echo-port port #:optional (outport (current-output-port)))
  (if (not (eof-object? (peek-char port)))
      (begin
	(display (read-line port 'concat) outport)
	(echo-port port outport))))

(define* (cat-port port 
		   #:optional (oport (current-output-port))
		   #:key (weight 1))
  (if (not (eof-object? (peek-char port)))
      (let ((port-line (read-line port)))
	(format oport "~a ~a~%" port-line weight)
	(cat-port port oport #:weight weight))))
